	...
	load() {
		...
		while (Object.values(this.konponents).some(konp => />_([a-zA-Z0-9_-]+)/.test(konp.content))) {
			for (const konponentName in this.konponents) {
				if (debugMode) { console.log(`Loading Konponent: ${konponentName}`); }
				...
				konp.content = konp.content.replace(regex, (match, p1) => {
					if (debugMode) { console.log(`Loading Konponent: ${konponentName} -> ${p1}`); }
		...
		for (const routeName in this.routes) {
			for (const konponentName in this.konponents) {
				if (this.routes[routeName].content.includes(`>_${konponentName}`)) {
					if (debugMode) { console.log(`Loading Konponent: ${routeName} -> ${konponentName}`); }
					...
					if (debugMode) { console.log(`Loading Konponent: ${routeName} -> ${konponentName} -> ${this.routes[routeName].content}`); }
				}
			}
		}
	}
